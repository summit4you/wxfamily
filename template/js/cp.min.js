...
alert("提交成功，获得了"+e.credit+"点积分");window.location.href="http://www.familyday.com.cn/wx/wx.php?do=feed&wxkey=$_GET[wxkey]";}else{alert(e.msg);}}});}function publicBlog(c,b,d,a){$.ajax({dataType:"jsonp",url:"http://www.familyday.com.cn/dapi/cp.php?ac=blog&m_auth="+encodeURIComponent(a)+"&message="+encodeURIComponent(b)+"&friend=0&come=wx&makefeed=1&blogsubmit=1&tags="+encodeURIComponent(d)+"&subject="+encodeURIComponent(c),success:function(e){if(e.error==0){e=e.data;
...
}}else{$("input[name=Filedata]").trigger("click");}});$("input[name=Filedata]").change(function(a){var b=a.target.files[0];$("#pic").remove();$.canvasResize(b,{width:640,height:0,crop:false,quality:60,callback:function(i,h,g,c){$("<img>").load(function(){$(this).appendTo(".msg-image");
$("#message").css({"padding-bottom":"0","margin-bottom":"0",height:$(this).height()});$(".msg-content").css({"padding-bottom":"0","margin-bottom":"0",height:$(this).height()});
}).attr("src",i).attr("id","pic");var e=new FormData();var d=$.canvasResize("dataURLtoBlob",i);d.name=b.name;if(c){e.append($("input").attr("name"),b,$("input[type=file]")[0].files[0].name);
}else{e.append($("input").attr("name"),d,$("input[type=file]")[0].files[0].name);}e.append("op","uploadphoto");e.append("topicid","0");e.append("pic_title","from wx");
e.append("m_auth",$("#auth").val());$.ajax({url:"http://www.familyday.com.cn/dapi/cp.php?ac=upload",type:"POST",data:e,dataType:"json",contentType:false,processData:false,beforeSend:function(f){f.setRequestHeader("pragma","no-cache");
...
});$("#message").css({"padding-bottom":"0","margin-bottom":"0",height:$("#pic").height()});$(".msg-content").css({"padding-bottom":"0","margin-bottom":"0",height:$("#pic").height()});
});